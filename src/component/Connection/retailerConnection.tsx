...
const GET_API_URL = `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/retailer-list?sellerId=${customId}`
const UPDATE_API_URL = `${process.env.NEXT_PUBLIC_BACKEND_URL}/user/update-retailer-list`
...
export default function RetailerConnectionTable() {
  ...
  // Fetch retailer list from GET API
  useEffect(() => {
    const fetchRetailers = async () => {
      try {
  ...
  // Handle block or suspend action (calls UPDATE API)
  const handleAction = async (id: number, action: 'block' | 'suspend') => {
    const retailer = retailers.find(r => r.id === id)
    console.log("rrrr", retailer)
    const updatedDroppedStatus = action === 'block' ? true : false  // Block -> dropped: true, Suspend -> dropped: false

    // Send the update request to the POST API
    try {
      const response = await fetch(UPDATE_API_URL, {
        ...
        body: JSON.stringify({
          customId: retailer?.customId, // Replace with the correct field if needed
          dropped: updatedDroppedStatus
        })
      })
      ...
      if (response.ok) {
        ...
        setRetailers((prevRetailers) => 
          prevRetailers.map((r) => 
            r.id === id ? { ...r, dropped: updatedDroppedStatus } : r
          )
        )
        setOpenDropdown(null)
      } else {
        console.error('Failed to update retailer status')
      }
    } catch (error) {
          ...
          <table className="min-w-full bg-white">
            ...
            <tbody>
              {retailers.map((retailer) => (
                <tr
                  key={retailer.id}
                  className={`border-b ${retailer.dropped
                    ? 'bg-red-100' // Red background if dropped is true
                    : '' // Default white background if dropped is false
                    }`}
                >
                  <td className="py-2 px-4 text-xs font-normal text-black">{retailer.name}</td>
                  <td className="py-2 px-4 text-[10px] font-normal text-black">{retailer.onboardedOn}</td>
